...
public class SpringConfigClass implements WebApplicationInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		/*
		 * 404error 가 나오고 콘솔에 onStartup 출력됨 현재 Spring MVC라이브러리를 모두 setting하고 있는 상테이며
		 * WebApplicationInitializer를 구현하고 있는 클래스가 있다면 자동으로 수행되므로 본래는 web.xml이 수행되어야 하지만
		 * 없으므로 SpringConfigClass가 작동되므로 404error여도 콘솔에 출력되는것임 -> 자바코드로 web.xml를 구현
		 */
